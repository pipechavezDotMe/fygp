  ...
  settings = jQuery.extend(
    {
      ...
      prevButtonOut: '#prevout', // value: any HTML tag or #id, default to #prev
      nextButtonOut: '#nextout', // value: any HTML tag or #id, default to #next
    ...
    if ($(settings.datesDiv).length > 0 && $(settings.issuesDiv).length > 0) {
      ...
      $(settings.datesDiv + ' a').click(function (event) {
        ...
        if (currentIndex - 1 < 0) {
          ...
          $(settings.nextButtonOut).fadeIn('fast');
          $(settings.prevButtonOut).fadeOut('fast');
        } else if (currentIndex === howManyDates - 1) {
          $(settings.prevButton).fadeIn('fast');
          $(settings.nextButton).fadeOut('fast');
          $(settings.prevButtonOut).fadeIn('fast');
          $(settings.nextButtonOut).fadeOut('fast');
        } else {
          $(settings.nextButton + ',' + settings.prevButton).fadeIn('slow');
          $(settings.nextButtonOut + ',' + settings.prevButtonOut).fadeIn(
            'slow'
          );
        }
      ...
      $(settings.nextButtonOut).click(function (event) {
        event.preventDefault();
      ...
      $(settings.prevButtonOut).click(function (event) {
        event.preventDefault();
        const currentSelectedDate =
          $('#dates a').index($('#dates a.selected')) || 0;
      ...
      $('#dates').bind(
        'wheel',
        debounce(function ({ originalEvent }) {
          const index = $('#dates a').index($('#dates a.selected')) || 0;
          if (checkScrollDirectionIsUp(originalEvent)) {
            if (
              index === 0 ||
              $('#dates a').index($('#dates a.selected')) <= howManyDates - 1
            ) {
              $(settings.nextButton).click();
              $(settings.nextButtonOut).click();
            }
          } else {
            if ($('#dates a').index($('#dates a.selected')) > 0) {
              $(settings.prevButton).click();
              $(settings.prevButtonOut).click();
            }
          }
        }, 5)
      );
